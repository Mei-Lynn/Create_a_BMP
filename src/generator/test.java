...
public class test {
	 ...
	 * valores numericos en bmps Java usa por defecto big endian, así que hay que
	 * leer desde el final para escribirlo en LE. Se usa ints en caso de que ocupe 4
	 * bits
	 * 
	 * @param value
	 * @return
	 ...
	 * Metodo necesario para crear little endians, es el formato estandar para
	 * valores numericos en bmps Java usa por defecto big endian, así que hay que
	 * leer desde el final para esribirlo en LE. Se usa shorts en caso de que ocupe
	 * 2 bits
	 * 
	 * @param value
	 * @return
	...
	private static byte[] shortToLittleEndianBytes(short value) {
		return new byte[] { (byte) (value & 0xFF), // Byte menos significativo
				(byte) ((value >> 8) & 0xFF) // Byte más significativo
		};
	}

	/**
	 * Genera el encabezado del archivo, 14 bytes con informacion sobre el propio
	 * archivo
	 * 
	 * @param tamaño
	 * @return
	...
	private static byte[] FileHeader(int tamaño) {
		...
		rt[4] = bytesTamaño[2];
		rt[5] = bytesTamaño[3];
		// NOTA: Estoy evitando usar bucles deliberadamente para mantener una claridad
		// visual de que se guarda en cada byte
		...
		// 10-11-12-13, offset. AKA cual es el primer byte relativo al aspecto de la
		// imagen, para un bmp es cuando termina la cabecera en el byte 54
	...
	private static byte[] BMPHeader(int anchura, int altura, int bitsPorPixel, int tamañoZonaPixeles) {
		...
		// 28-29, tamaño de los puntos, es cuantos bits se necesitan para definir el
		// color de un pixel por pantalla
		byte[] tamañoPuntos = shortToLittleEndianBytes((short) bitsPorPixel);
		rt[14] = tamañoPuntos[0];
		rt[15] = tamañoPuntos[1];

		// 30-31-32-33, compresión, en este ejercicio no hacemos ninguna así que se
		// mantiene en 0
		rt[16] = 0;
		rt[17] = 0;
		rt[18] = 0;
		...
		// 38-39-40-41, Resolución horizontal, no es muy importante y la podemos dejar a
		// 0
		rt[24] = 0;
		rt[25] = 0;
		rt[26] = 0;
		...
		// 46-47-48-49, tabla de colores, irrelevante porque estamos usando todos los
		// colores con 24 bits por pixel, se mantiene a 0
		rt[32] = 0;
		rt[33] = 0;
		rt[34] = 0;
		rt[35] = 0;
	...
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in)) { // Convenientemente declaramos el scanner con el try necesario para
													// manejar files
			...
			int tamañoZonaPixeles = altura * anchura * bitsPorPixel / 8; // todos los bits necesarios para mostrar cada
																			// pixel segun las dimensiones dadas por el
																			// usuario
			...
			try (FileOutputStream fos = new FileOutputStream(img)) {
				...
				int squareSize = Math.min(anchura, altura) / 2; // tamaño del cuadrado
				int inicio = (anchura - squareSize) / 2; // posición inicial del cuadrado en el eje X
